...
def test_read_tsv_non_existent_file(fakefs: FakeFilesystem):
    ...
    with pytest.raises(BIDSException, match=escape(f"Could not read TSV file {tsv_path}:")):
        df = _read_tsv_as_df(tsv_path)
...
def test_write_tsv_file_already_exists(fakefs: FakeFilesystem):
    ...
    with pytest.raises(BIDSException, match=escape(f"BIDS TSV file {tsv_path} can't be written as it already exists")):
        _write_rows_to_tsv(tsv_path=tsv_path, first_column_name="aaa", rows = [{"aaa": "bbb"}])
