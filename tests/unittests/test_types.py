...
def test_ids_prefix_with_empty_label():
    with pytest.raises(BIDSException, match=escape("BIDS subject id sub- had invalid label (in sub-<label>): BIDS label can't be empty")):
        SubjectId("sub-")
    with pytest.raises(BIDSException, match=escape("BIDS session id ses- had invalid label (in ses-<label>): BIDS label can't be empty")):
        SessionId("ses-")
